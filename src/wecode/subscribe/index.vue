  ...
  <div class="app">
    ...
    <div class="btm">
      <div style="width:90%;margin:0 atuo;">
        ...
        <div class="wxtx">温馨提示：请根据您实际需要进行预约，恶意预约将会受到限制。</div>
      </div>
      ...
      <el-dialog title="预约成功" :visible.sync="centerDialogVisible" width="5rem" center>
        <div class="dialog_img"></div>

        <div class="dialog_success">预约成功</div>
        <div class="dialog_success_tip">
          请您在一小时内到达，若不能及时到达，
          <br />请您取消预约，否则将会被记为恶意预约。
        </div>
    ...
    <div class="button" @click="submit">完成预约</div>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      rules: {
        name: [{ required: true, message: "请选择预约时间", trigger: "change" }]
      }
    };
  },
  mounted() {},
  methods: {
    ...
    submit() {
      ...
      let reserveEndTime = this.timeStamp(
        this.reserveDuration * 60 * 1000 + myDate
      ); //预约结束时间
      ...
      // 预约充电
      util
        .request(global.API_PREFIX + "/api/v1/charging/reserve", params)
        .then(res => {
